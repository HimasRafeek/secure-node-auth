...
class SecureNodeAuth {
  constructor(options = {}) {
    ...
    this.db = new DatabaseManager(this.options.connection);
  ...
  _validateOptions(options) {
    ...
    if (isNaN(merged.security.bcryptRounds) || merged.security.bcryptRounds < 4 || merged.security.bcryptRounds > 31) {
  ...
  addField(field) {
    ...
    if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(name)) {
      throw new Error(`Invalid field name: ${name}. Must start with letter or underscore and contain only alphanumeric characters and underscores.`);
    }

    // Prevent overriding system fields
    const reservedFields = ['id', 'email', 'password', 'createdAt', 'updatedAt', 'emailVerified', 'emailVerificationToken', 'resetPasswordToken', 'resetPasswordExpires', 'isActive'];
  ...
  async register(userData) {
    ...
    const existingUser = await this.db.findUserByEmail(userDataCopy.email, this.options.tables.users);
    if (existingUser) {
    ...
    if (this.options.security.requireEmailVerification && 
        this.emailService && 
  ...
  async logout(refreshToken) {
    ...
    const affectedRows = await this.db.revokeRefreshToken(refreshToken, this.options.tables.refreshTokens);
  ...
  async logoutAll(userId) {
    ...
    const affectedRows = await this.db.revokeAllUserTokens(userId, this.options.tables.refreshTokens);
  ...
  _defaultAuditLogger(event, data) {
    ...
    console.log(`[AUDIT ${timestamp}] ${event}:`, JSON.stringify({
      userId: data.userId,
      email: data.email,
